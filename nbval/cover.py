...
def _make_suffix(cov):
    ...
    if cov and cov.data_suffix is not None:
        # If True, we're getting a autogen suffix per iniitialize
        # These will be combined automaitcally later
...
def _merge_nbval_coverage_data(cov):
    ...
    This step is performed so that the
    """
    suffix = _make_suffix(cov)
    if suffix is True:
...
3. data_suffix is set to True:
   ...
   tests. We pass True as suffix to kernel, giving it's own unique
